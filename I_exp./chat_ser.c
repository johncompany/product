...
・新たなユーザがログインするたびに, 新しいソケットを生成する。ユーザ数が上限に達した場合, 警告文をサーバとユーザに表示し、新しいソケットが生成されないようにする。
・新しくログインしたユーザにはログインしている他のユーザの名前を送信する。一方, 既にログインしているユーザには, 新たにログインがあったことを通知する。
